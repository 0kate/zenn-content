...
## 事前知識
脆弱性の概要やパッチの変更内容が確認できた所で実際にもう少し深く調べていきたいが、ここまでで`ActionDispatch::Executor`やら`ActiveSupport::ExecutionWrapper`やら`ActiveSupport::IsolatedExecutionState`やら`ActiveSupport::CurrentAttributes`やら色々出てきて`Rails`入門したて人間には辛いものがあるので、少しだけこの辺の事前知識について調べて準備しておく。
...
**実行したい処理(run)** と **事後処理(complete)** を登録しておくことで、`ExecutionWrapper#run!`を実行時に「スレッド内でこのクラスが実行中かどうか」をチェックする **事前処理** の付与して実行してくれる。
文字通り、任意の処理に対するラッパーとして振る舞うオブジェクト。
...
`ActiveSupport::ExecutionWrapper`内で`ActiveSupport::IsolatedExecutionState.unique_id`を管理していた
  (`ExecutionWrapper.active`というHashMap内で、`IsolatedExecutionState.unique_id`をキーにbooleanを値にして管理している)
⬇
